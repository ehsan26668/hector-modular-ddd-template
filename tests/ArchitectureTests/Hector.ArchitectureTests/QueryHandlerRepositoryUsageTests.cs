...
public sealed class QueryHandlerRepositoryUsageTests
{
    [Fact]
    public void Should_NotDependOnDomainRepositories_When_ClassImplementsQueryHandler()
    {
        // Arrange
        ...
        Assert.True(
            failures.Count == 0,
            $"QueryHandlers must not depend on repositories. Failed assemblies: {string.Join(", ", failures)}");
    }
}